...
namespace DataProcessing
{
    internal class DataProcessing
    {
            ...
            Parallel.Invoke(() =>
            {
                timeList = new List<double>(
                    ParallelAssist.ParallelAssist.ForMulti(rawData, 0, length - 1, (x, i) =>
                    {
                        ...
                        {
                            if (errorTemp != "E002") lock (locker) errorTemp = "E002";
                        ...
                        else
                        {
                            ...
                            {
                                if (temp < 0 || temp > double.MaxValue * 0.001)//後の計算の都合上の処理
                                {
                                    if (errorTemp != "E006") lock (locker) errorTemp = "E006";
                                    return double.MinValue;
                                }
                                else return temp;
                            }
        ...
        internal void CheckTimeData(bool forceCheck = false)
        {
            ...
            int errorDataCount = timeList.Where(x => x < 0).ToList().Count;//0未満の時間を削除する
            int howManyDelete = 0;
            if (errorDataCount > 0 && error != "E007") error = "E007";
            for (int i = 0; i < errorDataCount - howManyDelete; i++)
            {
                timeList.RemoveAt(i);
                thrustList.RemoveAt(i);
                howManyDelete++;
                i--;
            }
            ...
            if (duplicates.Count > 10) isShowETC = "\netc...";
            while (duplicates.Count > 10) duplicates.RemoveAt(10);
            if (duplicates.Count > 0)
            {
                ...
                if (forceCheck)
                    if (MessageBox.Show("同じ時間を指すデータが存在します。このまま読み込みますか？\n\n重複する時間(s): \n" + string.Join("\n", duplicates.Select(x => x.ToString("0.000000"))) + isShowETC, "注意", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.No)
                    {
                        form1.graphInit.PerformClick();
                        return;
                    }
